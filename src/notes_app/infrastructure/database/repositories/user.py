...
class UserRepo(UserRepoInterface):
    ...
    async def add_user(self, user: UserEntity) -> UserEntity | None:
        user_orm = self._mapper.map_user_entity_to_orm(user)
        self._session.add(user_orm)
        await self._session.flush()
        await self._session.refresh(user_orm)
        return self._mapper.map_user_orm_to_entity(user_orm)
    ...
    async def get_user_by_user_id(self, user_id: int) -> UserEntity | None:
        ...
        if not user_orm:
            return None
        user_entity = self._mapper.map_user_orm_to_entity(user_orm)
        return user_entity
